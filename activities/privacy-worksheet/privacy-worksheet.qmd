...
4. A homogeneity attack exploits k-anonymity to establish that an individual has a protected attribute that takes one of a small number of values.
...
> __Example:__ Suppose the attacker knows that Jane Doe is 35 years old and that all women receiving HIV treatment on ward C are over 40. Alone, this information tells the attacker that Jane does not have HIV. However, in combination with the hospital records from previous example it _also_ reveals that Jane has been diagnosed with hepatitis and that other two women on ward C with HIV.
